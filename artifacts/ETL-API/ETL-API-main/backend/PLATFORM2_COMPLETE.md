...
curl -X POST -H "Authorization: Bearer <token>" \
  https://filter-connect.preview.emergentagent.com/api/dashboard/refresh
```
...
OpenAPI schema:
```
https://filter-connect.preview.emergentagent.com/openapi.json
```
...
```bash
# 1. Register user
curl -X POST https://filter-connect.preview.emergentagent.com/api/auth/register \
...
curl -X POST https://filter-connect.preview.emergentagent.com/api/admin/users/{user_id}/approve \
  -H "Authorization: Bearer <admin_token>"

# 3. User can now login
curl -X POST https://filter-connect.preview.emergentagent.com/api/auth/login \
  -H "Content-Type: application/json" \
...
curl -H "Authorization: Bearer <token>" \
  https://filter-connect.preview.emergentagent.com/api/opportunities/{opp_id}

# Update stage (stored as override)
curl -X PATCH https://filter-connect.preview.emergentagent.com/api/opportunities/{opp_id}/stage \
...
```bash
# Create goal
curl -X POST https://filter-connect.preview.emergentagent.com/api/goals \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
...
# Update progress
curl -X PATCH https://filter-connect.preview.emergentagent.com/api/goals/{goal_id}/progress \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
...
# Get summary stats
curl -H "Authorization: Bearer <token>" \
  https://filter-connect.preview.emergentagent.com/api/goals/summary/stats
```
