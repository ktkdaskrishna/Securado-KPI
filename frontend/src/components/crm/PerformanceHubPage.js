                  ...
                  <TableBody>{plans.map(p => (
                    <TableRow key={p.id} className={`cursor-pointer hover:bg-gray-50 ${selectedPlan?.id === p.id ? 'bg-blue-50' : ''}`} onClick={() => { setSelectedPlan(p); setTab('pm'); }}>
                      ...
                      <TableCell className="text-right font-mono text-sm font-semibold">
                        ...
                        {(p.margin_target > 0) && <div className="text-xs text-green-600">OMR {(p.margin_target || 0).toLocaleString()} <span className="text-[10px] text-green-400 font-normal">MARGIN</span></div>}
                      </TableCell>
