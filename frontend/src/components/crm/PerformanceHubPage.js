...
function CreatePlanDialog({ open, onClose, onCreated, productManagers }) {
  ...
  const handlePM = (n) => { const pm = productManagers.find(p => p.name === n); setForm(f => ({ ...f, product_manager_name: n, product_manager_id: String(pm?.id || ''), name: `Q1 2026 - ${n}` })); };
  const submit = async () => { if (!form.product_manager_name || !form.target_amount) { toast.error('Select PD and target'); return; } setSub(true); try { await targetAPI.createRevenuePlan(form); toast.success('Created'); onCreated(); } catch (err) { const detail = err.response?.data?.detail; const msg = Array.isArray(detail) ? detail.map(d => d.msg || d).join(', ') : (typeof detail === 'string' ? detail : 'Failed to create plan'); toast.error(msg); } finally { setSub(false); } };
