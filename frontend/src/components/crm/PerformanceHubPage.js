...
function CreatePlanDialog({ open, onClose, onCreated, productManagers }) {
  ...
  const submit = async () => { if (!form.product_manager_name || !form.target_amount) { toast.error('Select PD and target'); return; } setSub(true); try { await targetAPI.createRevenuePlan(form); toast.success('Created'); onCreated(); } catch { toast.error('Failed'); } finally { setSub(false); } };
