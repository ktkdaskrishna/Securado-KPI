...
function CreatePlanDialog({ open, onClose, onCreated, productManagers }) {
  const [form, setForm] = useState({ name: '', product_manager_name: '', product_manager_id: '', booking_target: 0, invoiced_target: 0, target_amount: 0, period: '2026-Q1' }); const [sub, setSub] = useState(false);
  ...
  return <Dialog open={open} onOpenChange={onClose}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Assign Revenue Target</DialogTitle></DialogHeader><div className="space-y-3"><div><Label className="text-xs text-gray-500">Product Director</Label><Select value={form.product_manager_name} onValueChange={handlePM}><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger><SelectContent>{productManagers.map(pm => <SelectItem key={pm.name} value={pm.name}>{pm.name} ({pm.opp_count} opps)</SelectItem>)}</SelectContent></Select></div><div className="grid grid-cols-2 gap-3"><div><Label className="text-xs text-gray-500">Booking Target (OMR)</Label><p className="text-[10px] text-gray-400 mb-1">Order booking from Won CRM deals</p><Input type="number" value={form.booking_target} onChange={e => setForm(f => ({ ...f, booking_target: parseFloat(e.target.value) || 0 }))} data-testid="booking-target-input" /></div><div><Label className="text-xs text-gray-500">Invoiced Target (OMR)</Label><p className="text-[10px] text-gray-400 mb-1">Revenue from paid invoices</p><Input type="number" value={form.invoiced_target} onChange={e => setForm(f => ({ ...f, invoiced_target: parseFloat(e.target.value) || 0 }))} data-testid="invoiced-target-input" /></div></div><div className="grid grid-cols-2 gap-3"><div><Label className="text-xs text-gray-500">Period</Label><Select value={form.period} onValueChange={v => setForm(f => ({...f, period: v, name: `${v.split('-')[1]} ${v.split('-')[0]} - ${f.product_manager_name || ''}`}))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="2026-Q1">Q1 2026</SelectItem><SelectItem value="2026-Q2">Q2 2026</SelectItem><SelectItem value="2026-Q3">Q3 2026</SelectItem><SelectItem value="2026-Q4">Q4 2026</SelectItem></SelectContent></Select></div><div><Label className="text-xs text-gray-500">Plan Name</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div></div></div><DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={submit} disabled={sub} className="bg-[#800000] hover:bg-[#9a1919] text-white">{sub ? '...' : 'Assign Target'}</Button></DialogFooter></DialogContent></Dialog>;
}
