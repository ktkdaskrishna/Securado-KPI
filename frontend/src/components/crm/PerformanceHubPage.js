...
function CreatePlanDialog({ open, onClose, onCreated, productManagers }) {
  ...
  const submit = async () => { if (!form.product_manager_name || !form.target_amount) { toast.error('Select PD and target'); return; } setSub(true); try { const res = await targetAPI.createRevenuePlan(form); toast.success('Plan created! Review suggested activities.'); onCreated(res.data); } catch (err) { const detail = err.response?.data?.detail; const msg = Array.isArray(detail) ? detail.map(d => d.msg || d).join(', ') : (typeof detail === 'string' ? detail : 'Failed'); toast.error(msg); } finally { setSub(false); } };
